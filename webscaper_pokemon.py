...
Absender = input("Von welcher E-Mail Adresse möchtest du senden? ")
Passwort = input("Wie ist das E-Mail Passwort für die Absenderadresse? ")
Empfänger = input("An welche E-Mail Adresse möchtest du senden? (mehrere mit Komma trennen) ")
...
def job(job_URL):
    if URL_Check(job_URL):
        Email_senden("Produkt wieder vorrätig", job_URL[0], Empfänger)
    else:
        print("Produkt nicht auf", job_URL[0])

# Alle 30 Minuten ausführen
for Eine_URL in URL_Liste:
    schedule.every(5).minutes.do(job, job_func:=Eine_URL)
